...
**Always read from `data`.** Don't rely on root-level keys — they're not part of the contract.
...
`armor-sets`, `locations`, and `regions` return every record in a single response and ignore `limit` and `offset`. They still wrap the result in `{ "data": [ ] }` for consistency.
