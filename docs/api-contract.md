...
| `name` | string | Human-readable location name (e.g. `"Cruma Tower"`, `"Ant Nest"`, `"Sea of Spores"`, `"Tower of Insolence"`). Whitespace-normalized at the public DTO boundary — leading/trailing spaces are trimmed and runs of internal whitespace collapsed to a single space. The generated `huntingzones.json` artifact preserves the source spelling verbatim; raw endpoints surface those names as-is. |
...
**Complementary to `RegionRef`, not a replacement.** Regions name the engine's death-teleport town (M4); locations name the local hunting ground (M7). A monster in the Outlaw Forest resolves to *Town of Schuttgart* on `primaryRegion` and *Outlaw Forest* on `primaryLocation`. `primaryRegion` stays coarse and independent of `primaryLocation`; the override carve-out below applies only to `primaryLocation`, never to `primaryRegion`.

**Override carve-out for `primaryLocation?` on NPC / Monster detail.** A small, audit-justified set of NPCs may opt out of the 2D nearest-anchor rule for their public `primaryLocation?` in `/npcs/[id]` and `/monsters/[id]` responses. The override is applied **only** in `toNpcDetailDto`; every other surface — the locations catalog, `EnrichedSpawnDto.location`, the raw endpoints, and `QuestDetailDto.primaryLocation?` — uses the unmodified resolver. Each entry corresponds to a verified case where the dungeon a boss inhabits has its center anchor at the entrance (often >10000 units from the deepest spawn) while a surface anchor sits closer in 2D, so the resolver picks the surface area instead. Returning the player-known dungeon name is more honest than returning either the surface anchor or `null`. Adding entries requires audit evidence (anchor distances, proof of the resolver mis-match, link from these docs); the map is not a general curation channel. **Current entries:** NPC `29001` *Queen Ant* → zone `34` *The Ant Nest*. See `PRIMARY_LOCATION_OVERRIDES_BY_NPC_ID` in [`src/lib/api/dto/location.ts`](../src/lib/api/dto/location.ts).
...
- **Override carve-out (NPC / Monster detail only)**: a small allow-list (`PRIMARY_LOCATION_OVERRIDES_BY_NPC_ID` in [`src/lib/api/dto/location.ts`](../src/lib/api/dto/location.ts)) maps specific NPC ids to a fixed hunting-zone id, bypassing the resolver. Applies only to `/npcs/[id]` and `/monsters/[id]` responses (both use the same DTO function). `QuestDetailDto.primaryLocation?`, `EnrichedSpawnDto.location`, the locations catalog, and the raw endpoints are unaffected. Current entry: NPC `29001` *Queen Ant* → zone `34` *The Ant Nest* — verified failure of the 2D nearest-anchor rule (Wasteland 8845 units beats The Ant Nest 12846 units). See the `LocationRefDto` section above for the audit policy.
- **Omission**: the field is **truly optional** (omitted, not `null`) when the NPC has no spawns at all OR every spawn falls outside the 10000-unit threshold of every anchor *and* no override entry exists OR the chronicle ships no `huntingzone-e.dat`. Same omission semantics as `primaryRegion?`.
