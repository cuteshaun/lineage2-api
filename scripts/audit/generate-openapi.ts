...
route({
  ...
  responses: listResponses(
    ...
    "Paginated reverse lookup: which NPCs drop this item. Empty page when the item exists but nothing drops it; 404 when the item id is unknown."
  ),
});
...
route({
  ...
  responses: listResponses(
    ItemSourceEntrySchema,
    "Paginated reverse lookup: which NPCs spoil this item. Empty page when the item exists but nothing spoils it; 404 when the item id is unknown."
  ),
});
...
route({
  ...
  responses: detailResponses(
    NpcDropsSchema,
    "All drops and spoil for this NPC. `drops` is empty when the NPC exists but has no drop table; 404 only when the NPC id is unknown."
  ),
});
...
route({
  ...
  responses: detailResponses(
    NpcDropsSchema,
    "All drops and spoil for this monster. `drops` is empty when the monster exists but has no drop table; 404 when the id is unknown or not a monster."
  ),
});
...
const document = generator.generateDocument({
  ...
  servers: [
    {
      url: "https://l2api.dev/api",
      description:
        "Production API root. All data paths are chronicle-scoped via the {chronicle} path parameter.",
    },
    {
      url: "/api",
      description:
        "Same-origin relative root (local development or a self-hosted deployment).",
    },
  ],
});
